...
- **2026-09-10 07:42 IST:** latest `main` commit is `b4c32c5d4dcf5c2e9d3c08b800d0bce671284f87` (`hardening: reject malformed Dodo webhooks before verification`). The commit changes only `app/api/webhooks/dodo/route.ts` and adds explicit required-header rejection before signature verification. GitHub's Vercel check for this commit is currently `failure` with the Vercel `build-rate-limit` condition, so this latest hardening is not yet verified as deployed.
- **2026-09-10 07:42 IST:** current Vercel production deployment is `dpl_HwCPrDMTN9y2qmsg31UoA4ZZqgqE`, state `READY`, serving commit `c1c4e4c7065de8d0b20bd64da49c74b4c9eca527` (`seo: keep root metadata neutral for private routes`). The latest production deployment is therefore behind `main`.
- **2026-09-10 07:42 IST:** production runtime log aggregation for the preceding two hours returned HTTP 200 responses only in the visible grouped status output; no 5xx runtime group was observed. This verifies only the observed Vercel runtime window, not payment success or complete application health.
- **2026-09-10 07:42 IST:** a direct deployment invocation was attempted through the available Vercel deployment interface, but that interface rejected the request because its exposed invocation requires `target`, `name`, and `files` parameters that are not available through the current action. No deployment result was fabricated.
- **2026-09-09 23:24 IST:** the previous production verification found no 5xx route groups in the preceding two hours. The only aggregated runtime error was the known Dodo webhook verification boundary error (`Missing required headers`) on `/api/webhooks/dodo`; this is an expected rejection path for unsigned/malformed webhook probes and does not establish a payment failure.
